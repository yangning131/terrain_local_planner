...
#define LANE_CHANGE_SPEED_FACTOR 0.5
    ...
    public:
        ...
        void run(tf::StampedTransform transform, nav_msgs::Path pathMsg, nav_msgs::Path& fixedGlobal, nav_msgs::Path& centerPath, nav_msgs::Path& remainingPath);
...
void RolloutGenerator::run(tf::StampedTransform transform, nav_msgs::Path pathMsg, nav_msgs::Path& fixedGlobal, nav_msgs::Path& centerPath, nav_msgs::Path& remainingPath, vector<nav_msgs::Path>& alternativePaths)
{
    ...
    getGlobalPlannerPath_cb(pathMsg);//赋速度、计算角度cost
...
void RolloutGenerator::run(tf::StampedTransform transform, nav_msgs::Path pathMsg, nav_msgs::Path& fixedGlobal, nav_msgs::Path& centerPath, nav_msgs::Path& remainingPath)
{
    current_pose.pos.x = transform.getOrigin().x();
    current_pose.pos.y = transform.getOrigin().y();
    current_pose.pos.z = transform.getOrigin().z();
...
void RolloutGenerator::getGlobalPlannerPath_cb(nav_msgs::Path msg)
{
    if (msg.poses.size() > 0) {
        ...
        msgLane2LocalLane(msg, single_path);//赋上速度
        calcAngleAndCost(single_path);//赋path cost
