...
class Node3D {
 public:
  ...
  /// set and get the index of the node in the 3D grid        int index_x = (int)round((x - occupancyMap2D.info.origin.position.x) / _mapResolution);
                                                                                                               //    int index_y = (int)round((y - occupancyMap2D.info.origin.position.y) / _mapResolution);
  int setIdx(int width, int height,int MapO_x,int MapO_y) { this->idx = (int)(t / Constants::deltaHeadingRad) * width * height + (int)(y- MapO_y) * width + (int)(x- MapO_x); return idx;}
  /// open the node
  ...
  bool isOnGrid(const int width, const int height ,const int MapO_x,const int MapO_y) const;
