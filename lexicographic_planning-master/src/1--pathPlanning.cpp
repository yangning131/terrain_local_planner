...
class PathPlanning : public ParamServer
{
   ...
   //int   visualization;
    ...
    void ImageMapCallback (const sensor_msgs::ImageConstPtr& image)
    {
        ...
        // ros::Time mapTime = image->header.stamp;
         ...
         ImageMap = cv_bridge::toCvShare(image,"")->image;
    }
   ...
   //}
    ...
    bool getRobotPosition()
    {
        ...
        // cv::Mat robot_thata = Chassis.descriptors[GetChassisdescriptorIndex(yaw)].image_;
               cv::Mat robot_thata = Chassis.descriptors[90].image_;
        ...
        // int test = imageROI.dot(robot_thata);
        std::cout<<"dot test: "<<std::endl;


        cv_bridge::CvImage out_z_image;
            // Same timestamp and tf frame as input image
            out_z_image.header   = std_msgs::Header(); // Same timestamp and tf frame as input imag
            ...
            cv::imshow("image",Chassis.descriptors[90].image_);
