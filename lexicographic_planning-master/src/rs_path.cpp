...
TypeVectorVecd<3> RSPath::GetRSPath(const Vect_3d &start_state, const Vect_3d &goal_state,
...
//              << rs_path.type_[4] << std::endl;
    ...
    for (unsigned int i = 0; i <= interpolation_number; ++i) {
        ...
        Vect_3d temp_pose(0.0, 0.0, start_state.z());
    ...
    // std::cout<<"pose size "<<path_poses.size()<<std::endl;
    return path_poses;
}