...
// possible movements  由运动学自行车模型求得
...
//                                         IS ON GRID
//###################################################
bool Node3D::isOnGrid(const int width, const int height ,const int MapO_x,const int MapO_y) const {
  return (x-MapO_x) >0 && x < width && (y-MapO_y) >0 && y < height && (int)(t / Constants::deltaHeadingRad) >= 0 && (int)(t / Constants::deltaHeadingRad) < Constants::headings;
  // return x>-width/4 && x < width/4 &&y>-height/4 && y < height/4 && (int)(t / Constants::deltaHeadingRad) >= 0 && (int)(t / Constants::deltaHeadingRad) < Constants::headings;
}
...
bool Node3D::isInRange(const Node3D& goal) const {
  ...
  float datatheata=std::abs(t-goal.t);
  if(dist<0.5&&datatheata<0.2)//10ｄｅｇｒｅｅ
  return true;
  return false;
}
