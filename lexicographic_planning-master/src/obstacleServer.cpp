...
public:
    ...
    obstacleServer()
    {
        ...
        pubImage_cost  = nh.advertise<sensor_msgs::Image> ("planning/obstacle/ImageMapCost", 1);
    ...
    void cloudHandler(const sensor_msgs::PointCloud2ConstPtr& laserCloudMsg)
    {
        ...
        if (pubOccupancyMap.getNumSubscribers() != 0||pubImage_collision.getNumSubscribers() > 0)
        {   
            ResetImgMat(img_mat_);
            ResetImgMat(img_mat_cost);
            std::cout<<"_local_map_grid_num obstacle :"<<_local_map_grid_num<<std::endl;
                ...
                for (int m = -search_num_ob; m <= search_num_ob; ++m)
                {
                    for (int n = -search_num_ob; n <= search_num_ob; ++n)
                    {
                        ...
                        else if(surroundMapCloudDS->points[i].intensity<chassis_hight)
                        {
                            ...
                            img_mat_.at<float>(x_id, y_id) = 20;
                        }
             ...
             if (img_mat_.type() == CV_32F)
               {
             ...
             else
                {
                 ...
                 // resize(img_mat, img_mat, Size(_local_map_grid_num,_local_map_grid_num ));

             out_z_image.image    = img_mat_; // Your cv::Mat

                //  std::cout<<"))))))))imageROI.rows: "<<img_mat_.rows<<"imageROI.cols: "<<img_mat_.cols<<" index"<<out_z_image.image.at<float>(10, 10)<<std::endl;
          ...
          if (pubOccupancyMap2.getNumSubscribers() != 0||pubImage_collision.getNumSubscribers() > 0)
        {
                        ...
                        for (int m = -search_num_field; m <= search_num_field; ++m)
                        {
                            for (int n = -search_num_field; n <= search_num_field; ++n)
                            {
                                ...
                                if (occupancyMap2D.data[index] != 100 && occupancyMap2D.data[index] != 70)
                                {
                                    ...
                                    img_mat_cost.at<float>(newIdX, newIdY) = occupancyMap2D.data[index] = max(occupancyMap2D.data[index], grid_value);
             ...
             if (pubImage_collision.getNumSubscribers() > 0)
          {
             ...
             out_z_image.header.frame_id   = "map"; // Same timestamp and tf frame as input image
                ...
                out_z_image.image    = img_mat_cost; // Your cv::Mat

             pubImage_cost.publish(out_z_image.toImageMsg());
          }
