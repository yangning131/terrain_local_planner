...
PurePursuit::PurePursuit() : ld_(1.0), v_max_(0.9), v_(v_max_), w_max_(1.0), pos_tol_(0.1), idx_(0),
                             ...
                             lookahead_frame_id_("lookahead")
{
  ...
  nh_private_.param<double>("linear_velocity", v_, 0.9);
...
void PurePursuit::computeVelocities(nav_msgs::Odometry odom)
{
  ...
  try
  {
    ...
    if (!goal_reached_)
    {
      ...
      double ld_2 = ld_ * ld_;
      // cmd_vel_.angular.z = std::min( 3.5*v_ / ld_2 * yt, w_max_ );
      double v_temp = 4.0*v_ / ld_2 * yt;
      if(v_temp > 0)
      {
          cmd_vel_.angular.z = std::min( 5.0*v_ / ld_2 * yt, 2.5 );
      }
      else
      {
        cmd_vel_.angular.z = std::max( 5.0*v_ / ld_2 * yt, -2.5 );
      }
