...
float terrainVoxelSize = 1.0;//1.0
int terrainVoxelShiftX = 0;
int terrainVoxelShiftY = 0;
const int terrainVoxelWidth = 41;//21
...
void laserCloudHandler(const sensor_msgs::PointCloud2ConstPtr &laserCloud2) {
  ...
  for (int i = 0; i < laserCloudSize; i++) {
    ...
    if (pointZ - vehicleZ > minRelZ - disRatioZ * dis &&
        ...
        dis < terrainVoxelSize * (terrainVoxelHalfWidth + 1)) {//一定距离的点云加入
      point.x = pointX;
      point.y = pointY;
      point.z = pointZ;
      point.intensity = laserCloudTime - systemInitTime;//两帧点云时间差
...
int main(int argc, char **argv) {
  ...
  ros::Subscriber subOdometry =
      nh.subscribe<nav_msgs::Odometry>("/odometry/filtered", 5, odometryHandler);

  // ros::Subscriber subLaserCloud = nh.subscribe<sensor_msgs::PointCloud2>(
  //     "/planning/registered_cloud", 5, laserCloudHandler);
  ...
  for (int i = 0; i < terrainVoxelNum; i++) {
    terrainVoxelCloud[i].reset(new pcl::PointCloud<pcl::PointXYZI>());//分配大小
  }

  downSizeFilter.setLeafSize(scanVoxelSize, scanVoxelSize, scanVoxelSize);//下采样 voxelsize
  ...
  while (status) {
    ...
    if (newlaserCloud) {
      ...
      float terrainVoxelCenX = terrainVoxelSize * terrainVoxelShiftX;//每一个voxel的中心点
      float terrainVoxelCenY = terrainVoxelSize * terrainVoxelShiftY;

      while (vehicleX - terrainVoxelCenX < -terrainVoxelSize) {//vehicleX 太靠左
      ...
      for (int i = 0; i < laserCloudCropSize; i++) {
        ...
        int indX = int((point.x - vehicleX + terrainVoxelSize / 2) /  //左下为0值
                       terrainVoxelSize) +
        ...
        if (indX >= 0 && indX < terrainVoxelWidth && indY >= 0 &&
          ...
          terrainVoxelCloud[terrainVoxelWidth * indX + indY]->push_back(point);//将1m内的点云push到一个坐标数组中
      ...
      for (int indX = terrainVoxelHalfWidth - 10;
        ...
        for (int indY = terrainVoxelHalfWidth - 10;
          ...
          *terrainCloud += *terrainVoxelCloud[terrainVoxelWidth * indX + indY];//周围5个index
        }
      }
      ...
      if (useSorting) {
        for (int i = 0; i < planarVoxelNum; i++) {
          int planarPointElevSize = planarPointElev[i].size();//里面的点云数量
